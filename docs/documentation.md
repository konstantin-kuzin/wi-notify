...
5. Если `reviewDesignTypes` непустой — тип исходной WI должен совпасть (без учёта регистра). Сейчас дефолт пустой → кнопка на **всех** типах.
6. Если тип не удалось прочитать из DOM, а список типов задан — поведение по умолчанию: **показывать**.
